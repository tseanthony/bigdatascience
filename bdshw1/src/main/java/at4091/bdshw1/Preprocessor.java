...
public class Preprocessor {
	...
	public Preprocessor() {
	    ...
	    props.setProperty("annotators", "tokenize,ssplit,pos,lemma,ner"); //,ner
...
//	    // UNCOMMENT TO PRINT NAMED ENTITY
//		for (int i = 0; i < exampleDocument.sentences().size(); i++) {
//			
//			List<CoreLabel> SentenceTokens = exampleDocument.sentences().get(i).tokens();
//			
//			for (int j = 0; j < SentenceTokens.size(); j++) {
//				
//	    	    // this is the text of the token
//	    	    String word = SentenceTokens.get(j).word();
//	    	    // this is the POS tag of the token
//	    	    String ne = SentenceTokens.get(j).ner();
//
...
//		}
	    ...
	    // Create nGram Map from CoreDocument 312, 422
	    //HashMap<String, Integer> nGramMap = createNGramNoNER(exampleDocument, maxSize, minSize, minWordFrequency);
	    //HashMap<String, Integer> nGramMap = createNGram(exampleDocument, maxSize, minSize, minWordFrequency);
...
//		int maxSize = 4;						// N-gram max size
//		int minSize = 2;						// N-gram min size
//		int minWordFrequency = 2;				// Frequency of token within document
...
//		int maxSize = 3;						// N-gram max size
//		int minSize = 1;						// N-gram min size
//		int minWordFrequency = 2;				// Frequency of token within document
			...
			for (int j = 0; j < SentenceTokens.size(); j++) {
				...
				for (int k = 0; k < dynamicMaxSize; k++) {
					...
					if (nGramMap.containsKey(wordStr)) {
						...
						// Add to dictionary if n-gram size is greater the minSize TEST
						if (wordCount >= minSize) nGramMap.put(wordStr, new Integer(1));	
